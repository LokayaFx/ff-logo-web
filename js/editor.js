...
// ========================
// LOGO GENERATION - AUTOMATIC DOWNLOAD
// ========================
function generateFinalLogo() {
    ...
    if (renderStatus) renderStatus.textContent = 'Initializing Photopea...';
...
function initializePhotopea(logoName, logoNumber, logoTitle) {
    ...
    // Correct URL format: https://www.photopea.com/#%7B%22files%22%3A%5B%22url%22%5D%7D
...
function executePhotopeaScript(logoName, logoNumber, logoTitle) {
    ...
    const escapedName = logoName.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    const escapedNumber = logoNumber.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    const escapedTitle = logoTitle.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    ...
    const script = 
        ...
        'try{d.artLayers.getByName("LogoName").textItem.contents="' + escapedName + '";}catch(e){}' +
        'try{d.artLayers.getByName("LogoNumber").textItem.contents="' + escapedNumber + '";}catch(e){}' +
        'try{d.artLayers.getByName("LogoTitel").textItem.contents="' + escapedTitle + '";}catch(e){}' +
        'setTimeout(function(){d.saveToOE("png");},500);' +
        '},3000);';
    ...
    setTimeout(function() {
        if (isWaitingForPNG) {
            console.error('❌ No response from Photopea');
            isWaitingForPNG = false;
            updateRenderStatus('Export timeout - please try again');
